...
categIRR<- readOGR("C:/Users/DELL/Downloads/MapIrr_shape/MapIrr_shape.shp",
...
catcat$label<-as.character(catcat$mergeirr_R)
catcat$label3<-as.character(catcat$NOM_ENT)
catcat$label4<-as.character(catcat$NOM_MUN)

catcat$label<-paste0("IRR:", catcat$label  , "| Entidad:",catcat$label3 , "| Municipio:",catcat$label4)

pal <- colorBin("viridis", domain = categIRR$mergeirr_R, bins = bins, reverse = TRUE)
pal

map_categIRR <- leaflet(categIRR) %>%
  addTiles() %>%
  addPolygons( weight = 0.0, smoothFactor = 0.5, stroke = TRUE, 
               opacity = 1.0, fillOpacity = 0.8,
               fillColor = ~pal(mergeirr_R),  color = "blue",
